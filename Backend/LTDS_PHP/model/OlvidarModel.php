...
class OlvidarModel {
    ...
    // Insertar nuevo token (SIN hashear para guardar)
    ...
    $stmt->execute([
        'num_doc' => $numDoc,
        'token' => $token,  // ← Guardamos sin hashear
    ...
    $stmt = $this->conn->prepare($query);
    $stmt->execute(['token' => $token]);  // ← Buscamos sin hashear
    ...
    $stmt = $this->conn->prepare($query);
    return $stmt->execute(['token' => $token]);  // ← Sin hashear
}
