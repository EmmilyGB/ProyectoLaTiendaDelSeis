    ...
    <div class="row mb-3 align-items-center g-2">
        ...
        <div class="col-12 col-lg-4 d-flex justify-content-lg-end">
            <form method="get" action="index.php">
                <input type="hidden" name="action" value="<?= $_GET['action'] ?? 'ofertas' ?>">
        ...
        <?php foreach ($productos as $producto): ?>
            <div class="col-6 col-md-4 product-col">
                <div class="card product-card h-100 position-relative">
                    <?php
                        $precioOriginal = isset($producto['PrecioOriginal']) ? (float)$producto['PrecioOriginal'] : (float)$producto['Precio'];
                        $precioOferta = isset($producto['PrecioOferta']) ? (float)$producto['PrecioOferta'] : null;
                        $tieneOferta = $precioOferta !== null && $precioOferta > 0 && $precioOferta < $precioOriginal;
                        $porcentaje = $tieneOferta ? (int)round((($precioOriginal - $precioOferta) / $precioOriginal) * 100) : 0;
                    ?>
                    ...
                    <a href="index.php?action=verProducto&id=<?= $producto['IdProducto'] ?>" class="text-decoration-none">
                        ...
                        <div class="card-body text-center">
                            ...
                            <?php if ($tieneOferta): ?>
                                ...
                                <p class="product-price mb-0">$<?= number_format($precioOferta, 0, ',', '.') ?></p>
                            <?php else: ?>
                                <p class="product-price mb-0">$<?= number_format($precioOriginal, 0, ',', '.') ?></p>
                            <?php endif; ?>
                        </div>
                    </a>
